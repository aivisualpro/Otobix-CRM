...
const GlobalImportModal = ({ 
  ...
  const [stats, setStats] = useState<ImportStats>({ total: 0, processed: 0, success: 0, failed: 0 });
      ...
      step: async (results: Papa.ParseStepResult<Record<string, unknown>>, parser: Papa.Parser) => {
        ...
        if (batch.length >= BATCH_SIZE) {
          ...
          try {
            ...
            const inserted = typeof result.inserted === 'number' ? result.inserted : (result.success ? result.count : 0);
            const failed = typeof result.failed === 'number' ? result.failed : (!result.success ? result.count : 0);
            ...
            <div className="flex flex-col items-center justify-center py-4">
              ...
              <div className="grid grid-cols-2 gap-4 w-full mb-6">
                <div className="bg-gray-50 p-3 rounded-lg text-center border border-gray-100">
                  <div className="text-2xl font-bold text-slate-800">{stats.processed.toLocaleString()}</div>
                  <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Processed</div>
                </div>
                <div className="bg-gray-50 p-3 rounded-lg text-center border border-gray-100">
                  <div className="text-2xl font-bold text-green-600">{stats.success.toLocaleString()}</div>
                  <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Success</div>
                </div>
              </div>
