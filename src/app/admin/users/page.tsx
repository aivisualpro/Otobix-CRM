...
import { Search, Plus, Edit2, Trash2, Download, X, User as UserIcon, Shield, Mail, MapPin, Lock, Phone, CheckCircle, Clock } from 'lucide-react';
...
const UserModal = ({ isOpen, onClose, onSubmit, editRecord }: UserModalProps) => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const addressList = addressRaw ? addressRaw.split(',').map(c => c.trim()).filter(Boolean) : [];

    // Parse allowed cities
    const citiesRaw = formData.get('allowedCities') as string;
    const allowedCities = citiesRaw ? citiesRaw.split(',').map(c => c.trim()).filter(Boolean) : [];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white shadow-2xl w-full max-w-lg border border-gray-100 rounded-xl">
        <div className="sticky top-0 bg-white/95 backdrop-blur-sm px-6 py-4 border-b border-gray-100 flex justify-between items-center z-10 rounded-t-xl">
          <h2 className="text-lg font-bold text-slate-900">{editRecord ? 'Edit User' : 'Add New User'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div>
            ...
            <div className="relative">
              ...
              <input name="userName" type="text" required className="form-input pl-9" defaultValue={editRecord?.userName} placeholder="John Doe" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <input name="email" type="email" required className="form-input pl-9" defaultValue={editRecord?.email} placeholder="john@example.com" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input name="phoneNumber" type="tel" className="form-input pl-9" defaultValue={editRecord?.phoneNumber} placeholder="1234567890" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <select name="userRole" className="form-input pl-9" defaultValue={editRecord?.userRole || 'User'}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select name="approvalStatus" className="form-input pl-9" defaultValue={editRecord?.approvalStatus || 'Pending'}>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">
              Password {editRecord && <span className="text-gray-400 font-normal">(Leave blank to keep unchanged)</span>}
            </label>
            <div className="relative">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
               ...
               <div className="relative">
                  ...
                  <input name="location" type="text" className="form-input pl-9" defaultValue={editRecord?.location} placeholder="Kolkata" />
               </div>
            </div>
            <div>
               <label className="block text-xs font-semibold text-slate-500 mb-1">Address List (Comma Sep)</label>
               <input name="addressList" type="text" className="form-input" defaultValue={editRecord?.addressList?.join(', ')} placeholder="Office, Home" />
            </div>
          </div>

          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">Allowed Cities (Comma Sep)</label>
            <textarea 
              name="allowedCities" 
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-2">
            ...
            <button type="submit" className="btn-primary">{editRecord ? 'Update User' : 'Create User'}</button>
          </div>
        </form>
      </div>
    </div>
