...
// All available user columns
// ... same as before but without GlobalImportModal ...
const ALL_COLUMNS = [
  { id: 'userName', label: 'User Name', defaultVisible: true },
  { id: 'email', label: 'Email', defaultVisible: true },
  { id: 'phoneNumber', label: 'Phone', defaultVisible: true },
  { id: 'userRole', label: 'Role', defaultVisible: true },
  { id: 'approvalStatus', label: 'Status', defaultVisible: true },
  { id: 'dealershipName', label: 'Dealership', defaultVisible: true },
  { id: 'location', label: 'Location', defaultVisible: true },
  { id: 'allowedCities', label: 'Allowed Cities', defaultVisible: false },
  { id: 'isStaff', label: 'Is Staff', defaultVisible: false },
  { id: 'entityType', label: 'Entity Type', defaultVisible: false },
  { id: 'primaryContactPerson', label: 'P. Contact Person', defaultVisible: false },
  { id: 'primaryContactNumber', label: 'P. Contact Number', defaultVisible: false },
  { id: 'secondaryContactPerson', label: 'S. Contact Person', defaultVisible: false },
  { id: 'secondaryContactNumber', label: 'S. Contact Number', defaultVisible: false },
  { id: 'addressList', label: 'Addresses', defaultVisible: false },
  { id: 'assignedKam', label: 'Assigned KAM', defaultVisible: false },
  { id: 'createdAt', label: 'Created At', defaultVisible: false },
  { id: 'rejectionComment', label: 'Rejection Comment', defaultVisible: false },
];
...
const STORAGE_KEY = 'users_columns_config';
        ...
        if (res.ok) {
          ...
          if (targetSetting && targetSetting.value) {
            savedConfigs = typeof targetSetting.value === 'string' 
              ...
              : targetSetting.value;
            console.log('[UsersSettings] Loaded config from MongoDB');
          }
        }
      } catch (err) {
        console.warn('[UsersSettings] MongoDB load failed, falling back to localStorage:', err);
      }
      ...
      if (!savedConfigs) {
        ...
        if (saved) {
          try {
            ...
            console.log('[UsersSettings] Loaded config from localStorage');
          } catch (err) {
            console.error('[UsersSettings] Error parsing local storage:', err);
          }
        }
      }
    ...
    try {
      ...
      const res = await fetch('/api/settings', {
        ...
        body: JSON.stringify({
          key: STORAGE_KEY,
          value: columns,
          label: 'User Columns Configuration',
      ...
      console.error('[UsersSettings] Save failed:', error);
    ...
    <div className="p-6">
      <div className="max-w-4xl mx-auto space-y-4">
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="user-columns" className="bg-white border rounded-xl overflow-hidden shadow-sm">
            <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-gray-50/50">
              <div className="flex flex-col items-start gap-1">
                <CardTitle className="text-lg">User Columns</CardTitle>
                <CardDescription>
                  Select and reorder columns displayed in the user list view ({visibleCount} of {columns.length} visible)
                </CardDescription>
              </div>
            </AccordionTrigger>
            <AccordionContent className="px-6 pb-6 pt-2">
              ...
              <p className="text-xs text-slate-500 mt-4">
                Drag and drop to reorder columns. Toggle visibility using the switch. Changes will apply after saving and refreshing the users page.
              </p>
            </AccordionContent>
          </AccordionItem>
        </Accordion>
