...
const DEFAULT_SETTINGS: Omit<Setting, '_id'>[] = [
  ...
  },
  {
    ...
    description: 'Default number of items in tables',
    type: 'number',
  },
  {
  ...
  },
  {
    ...
    description: 'Default priority for new records',
    type: 'select',
          ...
          <div className="max-h-44 overflow-y-auto">
            ...
            {filteredTypes.map((type) => (
              <button
                ...
                className={`w-full px-3 py-2 text-left text-sm hover:bg-blue-50 flex items-center justify-between ${value === type ? 'bg-blue-50 text-blue-700' : 'text-slate-700'}`}
              >
                {type}
          ...
          <div className="grid grid-cols-6 gap-2 mb-3">
            {PRESET_COLORS.map((color) => (
              <button
                ...
                className={`w-8 h-8 rounded-lg border-2 transition-transform hover:scale-110 ${value === color ? 'border-blue-500 ring-2 ring-blue-200' : 'border-transparent'}`}
    ...
    switch (setting.type) {
      case 'boolean':
        return (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg ${localValue ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}
          >
            {localValue ? (
              <>
      ...
      case 'json':
        return (
          <textarea
            value={
              typeof localValue === 'string' ? localValue : JSON.stringify(localValue, null, 2)
            }
            onChange={(e) => {
              try {
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] px-2 py-0.5 bg-gray-100 text-gray-500 rounded font-mono">
              {setting.key}
            </span>
          </div>
          {setting.description && (
...
interface AddSettingModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Omit<Setting, '_id'>) => void;
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white shadow-2xl w-full max-w-lg border border-gray-100 rounded-xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="form-input"
                value={selectedType}
                onChange={(e) => setSelectedType(e.target.value as Setting['type'])}
              >
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1">Default Value</label>
            {selectedType === 'boolean' ? (
              <select name="value" className="form-input">
                <option value="true">Enabled</option>
                <option value="false">Disabled</option>
              </select>
            ) : selectedType === 'json' ? (
              <textarea name="value" className="form-input h-20 font-mono text-xs" />
            ) : (
              <input
                name="value"
                ...
                {filteredDropdowns.length > 0 ? (
                  filteredDropdowns.map((row) => (
                    <tr key={row._id} className="border-b border-gray-50 hover:bg-gray-50/50">
                      ...
                      <td className="py-2 px-2 text-sm text-slate-600 whitespace-nowrap">
                        {row.type}
                      </td>
                      <td className="py-2 px-2">
                        <button
